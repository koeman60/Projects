    ...
    if (type === Actions.ChooseOp) {
      ...
      if (state.prevOp !== null && state.curOp !== 0) {
        ...
        return {
          ...state,
          operation: payload.operation,
          prevOp: evaluate(state).toString(),
          curOp: 0,
        };
      }
      if (state.curOp === 0) {
    ...
    <div className="App">
      <div className="calc-grid">
        <div className="output">
          <div className="prev">
            {FormatOperant(prevOp)} {operation} {FormatOperant(data)}
          </div>
          <div className="curr">{FormatOperant(curOp)}</div>
        </div>
        <button
          className="span-2"
